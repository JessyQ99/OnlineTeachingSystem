...
def teacherintro(teacherName=None):  # 教师介绍
    ...
    if not teacherinfo:
        ...
        return render_template('user/teacher-intro.html', imagename=teacherinfo.photourl, briefinfo=teacherinfo.briefinfo, details=teacherinfo.details, u_name=teacherinfo.u_name,
...
def answer():
    ...
    try:
        ...
        if value == None:
            ...
            return render_template('answer_value.html', answerset_id=answerset_id, queset_id=queset_id)
        my_connect.commit()
    except:
        # 发生错误时回滚
        my_connect.rollback()
...
def post_dis():
    ...
    print(number)
    sql2 = "insert into discussion (num,chapter,cid,uid,uname,content,time,replyto,isdeleted) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
    number, 5, c_id, u_id, name[0], content, current, -1, 0)
    print(sql2)
    cursor.execute(sql2)
    my_connect.commit()
    cursor.close()
